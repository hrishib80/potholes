const Auth = {
    ...
    async check() {
        try {
            ...
            if (res.ok) { const data = await res.json(); this.user = data.user; }
            else { this.user = null; }
            this.updateUI();
    ...
    updateUI() {
        ...
        if (this.user) {
            ...
            if (userMenu) { userMenu.style.display = 'flex'; document.getElementById('usernameDisplay').textContent = this.user.username; }
